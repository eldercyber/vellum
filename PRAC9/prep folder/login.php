<?php
	...
	if( isset($_POST['submit']) ){
		...
		$loginquery = $conn -> prepare("SELECT * FROM accounts WHERE lower(userName) = '$cleanedUsername' LIMIT 1");
		$loginquery->execute();
		$loginresult = $loginquery->fetchAll();
		
		if(isset($loginresult[0]['userName']) && !strcmp($cleanedPassword, $loginresult[0]['passWD'])){
