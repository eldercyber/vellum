<?php
	...
	if( isset($_POST['submit']) ){
		...
		if(strcmp($cleanedPassword, $cleanedCPassword)){
			...
			$sql = "SELECT COUNT(*) FROM accounts WHERE lower(userName) = '$cleanedUsername'";
			$checkusername = $conn -> query($sql);
			
			if ($checkusername->fetchColumn()) {
				array_push($page_messages, "ERROR: That username currently exists.");
			} else {
				$create_account = $conn -> prepare("INSERT INTO `accounts`(`userName`,`passWD`,`ipaddress`) VALUES ('$cleanedUsername','$cleanedPassword',NULL)");
...
<html>
	<body>
		...
		<form method="post">
			...
			<button onclick="location.href = 'login.php';" id="register-account" >Cancel</button>
		</form>
	</body>
</html>